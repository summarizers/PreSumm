...
class BertData:
    ...
    def preprocess(
        ...
        original_src_txt = [
            " ".join(s) for s in src
        ]  # original_src_txt: list of str(each sentence)

        idxs = [
            i
            for i, s in enumerate(src)
            if (len(s) > self.args.min_src_ntokens_per_sent) or (i == 0)
        ]  # 너무 짧은 문장은 제외
        ...
        ]  # truncate sentence by max_arc_ntokens_per_sent
        ...
        ]  # _segs: [sep] token index list
        segs = [
            _segs[i] - _segs[i - 1] for i in range(1, len(_segs))
        ]  # num of tokens in a sent
        segments_ids = []
...
def jsonls_to_dfs(from_dir, to_dir):
    ...
    for jsonl_path in tqdm(jsonl_paths):
        filename = os.path.splitext(os.path.basename(jsonl_path))[
            0
        ]  ## extract file name
...
def _df_to_jsons(df, prefix, to_dir, n_cpus, clients):
    ...
    for i, first_row_idx in enumerate(first_row_idx_list):
        ...
        last_row_idx_str = (digits_num - len(str(last_row_idx - 1))) * "0" + str(
            last_row_idx - 1
        )
    ...
    for result in pool.imap_unordered(
        _df_to_json, zip(df_list, file_name_list, client_list)
    ):  # 길이가 작은거에 맞춰서 중단됨.
        print(result)
    pool.close()
    pool.join()
...
def _df_to_json(params):
    ...
    for i, row in df.iterrows():  # df.iloc[idx:end_idx].iterrows():
        ...
        if text_len > 100000:
            print(
                f"Request is too long: {text_len} characters. Max length is 100000 characters."
            )
            continue
        try:
...
def _json_to_bert(json_file, bert_file, args):
    ...
    for d in jobs:
        ...
        b_data = bert.preprocess(
            ...
            use_bert_basic_tokenizer=args.use_bert_basic_tokenizer,
            is_test=is_test,
            seg_embedding_type=args.seg_embedding_type,
        )
        ...
        datasets.append(b_data_dict)
    logger.info("Processed instances %d" % len(datasets))
    logger.info("Saving to %s" % bert_file)
    torch.save(datasets, bert_file)
    datasets = []
    gc.collect()
